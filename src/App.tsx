...
export default function App() {
  ...
  const [currentTimetableId, setCurrentTimetableId] =
    useState("1");
  ...
  useEffect(() => {
    ...
    console.log(
      "  - assignments with addedToAI:",
      assignments.filter((a) => a.addedToAI).length,
    );

    // 타이머를 사용하여 초기 렌더링 이후에 실행
    const timer = setTimeout(() => {
      if (schedule.length > 0 && assignments.length > 0) {
        const aiAssignments = assignments.filter(
          (a) => a.addedToAI && !a.completed,
        );
        ...
        if (schedule.length === 0) {
          console.log(
            "  ❌ schedule이 비어있음 - currentTimetable 확인 필요",
          );
        }
  ...
  const handleScheduleAnalyzed = (
    analyzedSchedule: TimeSlot[],
  ) => {
  ...
  const handleAddToAI = (assignmentIds: string[]) => {
    ...
    console.log(
      ...
      assignmentIds.map((id) => {
        ...
        return assignment
          ? `${assignment.title} (${assignment.estimatedTime}분)`
          : id;
      }),
    );
    ...
    const addedCount = updated.filter(
      (a) => a.addedToAI,
    ).length;
    console.log(
      "✅ 업데이트 후 AI에 추가된 전체 과제 수:",
      addedCount,
    );
    console.log(
      "📊 업데이트된 assignments:",
    ...
    if (schedule.length === 0) {
      ...
      toast.warning(
        `${assignmentIds.length}개 과제가 AI에 추가되었습니다!\n시간표를 업로드하면 추천 일정이 생성됩니다.`,
        {
          duration: 4000,
        },
      );
      ...
      console.log(
        "✅ 시간표 있음, useEffect에서 자동으로 추천 재생성됨",
      );
      toast.success(
        `${assignmentIds.length}개 과제가 AI에 추가되었습니다!\nAI 탭에서 추천 일정을 확인하세요.`,
        {
  ...
  const handleProgressUpdate = (
    ...
    const updated = assignments.map((a) =>
      a.id === assignmentId ? { ...a, completed, progress } : a,
    );
  ...
  const generateRecommendations = (
    ...
    console.log(
      "📌 [디버깅 1] freeSlots.length:",
      freeSlots.length,
    );
    console.log(
      "📌 [디버깅 2] assignmentData.length:",
      assignmentData.length,
    );
    ...
    const pendingAssignments = assignmentData
      ...
      .map((a) => {
        ...
        const remainingTime = Math.ceil(
          (a.estimatedTime * (100 - progress)) / 100,
        );
        console.log(
          `  �� 과제: "${a.title}" - estimatedTime: ${a.estimatedTime}분, progress: ${progress}%, remainingTime: ${remainingTime}분`,
        );
    ...
    console.log(
      "📌 [디버깅 3] pendingAssignments.length:",
      pendingAssignments.length,
    );
    console.log(
      "📋 pendingAssignments (addedToAI=true, completed=false, remainingTime>0):",
      pendingAssignments,
    );
    ...
    for (const freeSlot of freeSlots) {
      ...
      if (preferences) {
        ...
        if (
          ...
          const shouldAvoid = preferences.avoidTimeSlots.some(
            (avoid) => {
              ...
              if (overlap) {
                console.log(
                  `    ⛔ avoidTimeSlots 필터링: ${avoid.startTime}-${avoid.endTime}와 겹침`,
                );
              }
              return overlap;
            },
        ...
        if (
          ...
          const isPreferred =
            preferences.preferredTimeSlots.some((preferred) => {
              const overlap = timeOverlaps(
                freeSlot.startTime,
                freeSlot.endTime,
                preferred.startTime,
          ...
          if (!isPreferred) {
            console.log(
              `    ⚠️ 건너뜀: preferredTimeSlots 설정 있으나 선호 시간대 아님`,
            );
            console.log(
              `      설정된 선호 시간대:`,
              preferences.preferredTimeSlots,
          ...
          console.log(
            `    ✅ 선호 시간대 미설정 또는 비어있음, 모든 시간 허용`,
          );
        }
      } else {
        console.log(
          `    ✅ preferences 미설정, 모든 시간 허용`,
        );
      }
      ...
      const slotDuration = calculateDuration(
        freeSlot.startTime,
        freeSlot.endTime,
      );
      console.log(
        `    ⏱️ 슬롯 길이: ${slotDuration}시간 (${slotDuration * 60}분)`,
      );

      // 먼저 정확히 맞는 과제를 찾고, 없으면 더 작은 과제를 찾음
      let suitableAssignment = pendingAssignments.find(
        (assignment) => {
          const notUsed = !usedAssignments.has(assignment.id);
          const fits =
            assignment.remainingTime <= slotDuration * 60;

          if (!notUsed) {
            console.log(
              `      ⏭️ "${assignment.title}": 이미 사용됨`,
            );
          } else if (!fits) {
            console.log(
              `      ⏭️ "${assignment.title}": 시간 부족 (필요: ${assignment.remainingTime}분 > 가능: ${slotDuration * 60}분)`,
            );
          } else {
            console.log(
              `      ✅ "${assignment.title}": 매칭 성공! (필요: ${assignment.remainingTime}분 <= 가능: ${slotDuration * 60}분)`,
            );
          }
      ...
      if (suitableAssignment) {
        ...
        console.log(
          `    ➕ 추천에 추가됨 (총 ${newRecommendations.length}개)`,
        );
      } else {
    ...
    if (newRecommendations.length === 0) {
      ...
      if (freeSlots.length === 0) {
        console.log(
          "  ❌ 문제: 공강 슬롯이 없습니다. 시간표를 확인하세요.",
        );
      } else if (pendingAssignments.length === 0) {
        console.log(
          "  ❌ 문제: AI에 추가된 미완료 과제가 없습니다.",
        );
      } else if (
        skippedByPreferences > 0 &&
        skippedByPreferences === freeSlots.length
      ) {
        console.log(
          "  ❌ 문제: 모든 공강 슬롯이 선호 시간대 필터로 제외되었습니다.",
        );
        console.log(
          "  💡 해결방법: 설정에서 선호 시간대를 조정하거나 제거하세요.",
        );
      } else if (skippedByAvoid === freeSlots.length) {
        console.log(
          "  ❌ 문제: 모든 공강 슬롯이 피하고 싶은 시간대로 제외되었습니다.",
        );
        console.log(
          "  💡 해결방법: 설정에서 피하고 싶은 시간대를 조정하세요.",
        );
      } else {
        console.log(
          "  ⚠️ 문제: 공강 슬롯과 과제가 있지만 매칭되지 않습니다.",
        );
        console.log("  💡 가능한 원인:");
        console.log(
          "     - 모든 과제의 소요 시간이 공강 슬롯보다 김",
        );
        ...
        if (
          ...
          const slotDurations = freeSlots.map(
            (slot) =>
              calculateDuration(slot.startTime, slot.endTime) *
              60,
          );
          const maxSlotDuration = Math.max(...slotDurations);
          const minAssignmentTime = Math.min(
            ...pendingAssignments.map((a) => a.remainingTime),
          );

          console.log(
            `  📊 가장 긴 공강 슬롯: ${maxSlotDuration}분`,
          );
          console.log(
            `  📊 가장 짧은 과제 시간: ${minAssignmentTime}분`,
          );

          if (minAssignmentTime > maxSlotDuration) {
            console.log(
              "  ❌ 원인 확인: 모든 과제가 가장 긴 공강 슬롯보다 깁니다.",
            );
            console.log(
              "  💡 해결방법: 과제를 더 작은 단위로 나누거나, 예상 시간을 줄이세요.",
            );
          }
        }
      }
    ...
    days.forEach((day) => {
      ...
      schoolHours.forEach((hour) => {
        const hasClass = daySchedule.some((slot) => {
          ...
          if (overlap) {
            console.log(
              `    ❌ ${hour.start}-${hour.end}: ${slot.subject}와 겹침`,
            );
          }
          return overlap;
        });
          ...
          >
            ...
            {viewMode === "monthly" && (
              <MonthlyView
                ...
                currentDate={currentDate}
                hideClasses={
                  preferences?.hideClassesInMonthly || false
                }
                assignments={assignments}
              />
            )}
